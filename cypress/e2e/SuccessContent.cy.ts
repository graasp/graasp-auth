...
describe('Success Content Validation', () => {
  it('Sign In - Back Button', () => {
    ...
    cy.get(`#${SUCCESS_CONTENT_SIGN_IN_ID}`).should('not.exist');

    // Siging in with a valid email
    cy.signInAndCheck(GRAASP);

    cy.get(`#${SUCCESS_CONTENT_SIGN_IN_ID}`).should('be.visible');
  ...
  it('Sign Up - Back Button', () => {
    ...
    cy.get(`#${SUCCESS_CONTENT_SIGN_UP_ID}`).should('not.exist');
